...
const getByClubsId = async (slug) => {
    const result = await axiosInstance.get(`club/${slug}`);
    return result.data;
};
...
const fetchAvantageDetails = async (slug) => {
    const result = await axiosInstance.get(`avantage-details/${slug}`);
    return result.data;
};
const fetchAvantageKampanya = async (slug) => {
    const result = await axiosInstance.get(`avantage-kampanya/${slug}`);
    return result.data;
};
const fetchAvantageSliders = async (slug) => {
    const result = await axiosInstance.get(`avantage-slider/${slug}`);
    return result.data;
};
const exportFunction = {
    fetchAvantageSliders,
    fetchAvantageKampanya,
    fetchAvantageDetails,
    getByTalks,
    fetchTalking,
    fetchEvents,
    getByEventsId,
